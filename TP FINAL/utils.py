...
def get_data(base_url, endpoint,api_key, data_field=None, params=None, headers=None):
    """
    ...
    """
    try:
        endpoint_url = f"{base_url}{endpoint}{api_key}"
...
def read_most_recent_partition_data(data_path):
    """
    Lee la particion más reciente de una tabla Delta Lake, teniendo en cuenta
    la fecha y hora actual.
    Supone que la tabla Delta Lake está particionada por fecha y hora.

    Args:
      data_path (str): La ruta donde se encuentra la tabla Delta Lake.
      
    Returns:
      pd.DataFrame: Los datos más recientes de la tabla Delta Lake.
    """
    try:
        ...
        dt = DeltaTable(data_path)
        df_all  = dt.to_pandas()
        # De toda la tabla Delta lake, se filtran los datos de la fecha solicitada
        df_recent = df_all [
                (df_all ['date_extraction'] == requested_date.strftime("%Y-%m-%d"))
            ]
        
        return df_recent

    except TableNotFoundError:
        print("No se encontró la tabla Delta Lake")
        return None